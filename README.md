...
| 案件 | 車種 | 判定 | スコア | 検出根拠 |
|---|---|---|---|---|
| CLM-2024-0003 | ノート E13 | 要精査 | 70 | 前方衝突なのにリヤバンパフェイシアを計上（架空計上）／ボンネットフード 106,000円 vs 定価 89,100円（+19%）／マスタ未登録の部品番号 |
| CLM-2024-0005 | ヤリスクロス | 要精査 | 55 | 左フロントフェンダ板金 指数9.5 vs 標準3.8（+150%）／画像にないフロントバンパカバーを計上 |
...
**Cowork**（旧 Snowflake Intelligence）から「協定業務アシスタント」を選んで試してください。

1. **リスクスコアが最も高い案件はどれですか。その理由も教えてください。**
   → `claim_lookup` ツール。CLM-2024-0003 が返ります。
2. **CLM-2024-0004 の見積で価格乖離がある明細を教えてください。**
   → `fraud_analysis` ツール。リヤバンパフェイスの +25% が返ります。
3. **修理期間中の代車費用は約款上支払対象になりますか。**
   → `policy_search` ツール。第18条（代車費用特約）が引用されます。
