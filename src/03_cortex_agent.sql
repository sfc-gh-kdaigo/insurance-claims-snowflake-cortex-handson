...
instructions:
  response: |
    ...
    - policy_search を使って回答を生成した場合は、参照した検索結果のチャンクを
      引用元として必ず本文に表示してください。条文番号（第N条）とセクション名を添え、
      該当箇所の原文を引用したうえで、それに基づく解釈を記述してください。
    - 回答、および Thinking Step は日本語とし、構造化して根拠を含めて記述してください。
